...
def save_ghostmaps(original, outputmaps):
    ...
    ghostmap_filepath = os.path.join('ghostmaps_results/Columbia', os.path.basename(original) + '_ghostmaps.png')
    plt.savefig(ghostmap_filepath)

def convert_mask_colored_to_gray(mask_path):
    mask_image = cv2.imread(mask_path) 
    gray_mask = np.zeros(mask_image.shape[:2], dtype=np.uint8) 
    
    #identify green pixels 
    green_pixels = (mask_image[:,:,1] > 150) & (mask_image[:,:,1] > mask_image[:,:,0]) & (mask_image[:,:,1] > mask_image[:,:,2])
    
    #identify red pixels 
    red_pixels = (mask_image[:,:,2] > 150) & (mask_image[:,:,2] > mask_image[:,:,0]) & (mask_image[:,:,2] > mask_image[:,:,1])
    
    gray_mask[green_pixels] = 255
    gray_mask[red_pixels] = 0
...
if __name__ == '__main__':
    ...
    with open('results_ghostmaps_Columbia.json', 'w') as f:
        json.dump({}, f)
    #create ghostmap output map
    os.makedirs('ghostmaps_results/Columbia', exist_ok=True)
    ...
    with open('results_probability_3x3_Columbia.json', 'w') as f:
        json.dump({}, f)
    #create probmap segmented 3x3 output map
    os.makedirs('probability_map_3x3_results/Columbia', exist_ok=True)
    ...
    with open('results_noise_Columbia.json', 'w') as f:
        json.dump({}, f)
    #create noise output map
    os.makedirs('noisemap_results/Columbia', exist_ok=True)
    ...
    with open('results_MM-fusion_Columbia.json', 'w') as f:
        json.dump({}, f)
    #create noise output map
    os.makedirs('MM-fusion_results/Columbia', exist_ok=True)
    ...
    with open('results_DFAD2023_Columbia.json', 'w') as f:
        json.dump({}, f)
    #create noise output map


    #create error log json file, so we can store results in file each time, preserving results in case of unexpected interupt
    with open('error_log_Columbia.json', 'w') as f:
        json.dump('hopefully this remains empty', f)
        f.write('\n')


    #Columbia dataset contains pristine and manipulated images
    columbia_auth_dir = "Columbia/4cam_auth"
    columbia_splc_dir = "Columbia/4cam_splc"
    columbia_mask_dir = "Columbia/4cam_splc/edgemask"

    original_images = [os.path.join(columbia_auth_dir, path) for path in os.listdir(columbia_auth_dir) if not os.path.isdir(os.path.join(columbia_auth_dir, path))]
    manipulated_images = [os.path.join(columbia_splc_dir, path) for path in os.listdir(columbia_splc_dir) if not os.path.isdir(os.path.join(columbia_splc_dir, path))]
    mask_files_colored = [os.path.join(columbia_mask_dir, path) for path in os.listdir(columbia_mask_dir) if not os.path.isdir(os.path.join(columbia_mask_dir, path)) and "3." not in path]

    #remember to convert red/green masks to grayscale
    ...
    try:
        ...
        for image_path in original_images:
            ...
            result_data = {
                ...
                "image outputmaps name": os.path.join('ghostmaps_results/Columbia', os.path.basename(image_path) + '_ghostmaps.png'),
            ...
            with open('results_ghostmaps_Columbia.json', 'a') as f:
        ...
        for image_path in manipulated_images:
            ...
            mask_path = mask_files_colored[mask_counter]
            ...
            mask = convert_mask_colored_to_gray(mask_path)
            ...
            for i in range(n_ghostmaps-2*edge):
                ...
                if  len(values_inside_mask) == 0: #it can occur that the mask resizing deletes the mask area, in that= the entire mask becomes black or white; in such cases make top and bottom row different and log to error
                    ...
                    with open('error_log_Columbia.json', 'a') as f:
    ...
    for image_path in original_images:
        ...
        probmap_filepath = os.path.join('probability_map_3x3_results/Columbia', os.path.basename(image_path) + '_probmap.png')
        ...
        with open('results_probability_3x3_Columbia.json', 'a') as f:
            json.dump(result_data, f)
            f.write('\n')
    ...
    try:
        ...
        for image_path in original_images:
            ...
            noisemap_filepath = os.path.join('noisemap_results/Columbia', os.path.basename(image_path) + 'noisemap.png')
            ...
            noisemap_2_filepath = os.path.join('noisemap_results/Columbia', os.path.basename(image_path) + '_noisemap_2.png')
            ...
            with open('results_noise_Columbia.json', 'a') as f:
                json.dump(result_data, f)
                f.write('\n')
   ...
   #calculate expired time and write to error log
    ...
    try:
        ...
        detection_scores, map_locations = process_images(list_absolute_image_paths, 'MM-fusion_results/Columbia/')
        ...
        for image in original_images:
            ...
            with open('results_MM-fusion_Columbia.json', 'a') as f:
                json.dump(result_data, f)
                f.write('\n')
            mm_counter += 1
    ...
    try:
        ...
        for image in original_images:
            ...
            with open('results_DFAD2023_Columbia.json', 'a') as f:
                json.dump(result_data, f)
                f.write('\n')
            mm_counter += 1
