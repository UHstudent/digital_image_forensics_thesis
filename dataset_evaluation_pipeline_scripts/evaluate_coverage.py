...
def save_ghostmaps(original, outputmaps):
    ...
    ghostmap_filepath = os.path.join('ghostmaps_results/Coverage', os.path.basename(original) + '_ghostmaps.png')
...
if __name__ == '__main__':
    ...
    with open('results_ghostmaps_Coverage.json', 'w') as f:
        json.dump({}, f)
    #create ghostmap output map
    os.makedirs('ghostmaps_results/Coverage', exist_ok=True)
    ...
    with open('results_probability_3x3_Coverage.json', 'w') as f:
        json.dump({}, f)
    #create probmap segmented 3x3 output map
    os.makedirs('probability_map_3x3_results/Coverage', exist_ok=True)
    ...
    with open('results_noise_Coverage.json', 'w') as f:
        json.dump({}, f)
    #create noise output map
    os.makedirs('noisemap_results/Coverage', exist_ok=True)
    ...
    with open('results_MM-fusion_Coverage.json', 'w') as f:
        json.dump({}, f)
    #create noise output map
    os.makedirs('MM-fusion_results/Coverage', exist_ok=True)
    ...
    with open('results_DFAD2023_Coverage.json', 'w') as f:
        json.dump({}, f)
    #create noise output map


    #create error log json file, so we can store results in file each time, preserving results in case of unexpected interupt
    with open('error_log_Coverage.json', 'w') as f:
        json.dump('hopefully this remains empty', f)
        f.write('\n')


    #Coverage dataset contains pristine and manipulated images
    coverage_image_dir = "Coverage/image"
    coverage_mask_dir = "Coverage/mask"

    image_files = [os.path.join(coverage_image_dir,path) for path in os.listdir(coverage_image_dir)]
    original_images = []
    manipulated_images = []
    ...
    mask_paths = [os.path.join(coverage_mask_dir,path) for path in os.listdir(coverage_mask_dir) if "forged" in path]
    ...
    try:
        ...
        for image_path in original_images:
            ...
            result_data = {
                ...
                "image outputmaps name": os.path.join('ghostmaps_results/Coverage', os.path.basename(image_path) + '_ghostmaps.png'),
            ...
            with open('results_ghostmaps_Coverage.json', 'a') as f:
        ...
        for image_path in manipulated_images:
            ...
            for i in range(n_ghostmaps-2*edge):
                ...
                if  len(values_inside_mask) == 0: #it can occur that the mask resizing deletes the mask area, in that= the entire mask becomes black or white; in such cases make top and bottom row different and log to error
                    ...
                    with open('error_log_Coverage.json', 'a') as f:
    ...
    for image_path in original_images:
        ...
        probmap_filepath = os.path.join('probability_map_3x3_results/Coverage', os.path.basename(image_path) + '_probmap.png')
        ...
        with open('results_probability_3x3_Coverage.json', 'a') as f:
            json.dump(result_data, f)
            f.write('\n')
    ...
    try:
        ...
        for image_path in original_images:
            ...
            noisemap_filepath = os.path.join('noisemap_results/Coverage', os.path.basename(image_path) + 'noisemap.png')
            ...
            noisemap_2_filepath = os.path.join('noisemap_results/Coverage', os.path.basename(image_path) + '_noisemap_2.png')
            ...
            with open('results_noise_Coverage.json', 'a') as f:
                json.dump(result_data, f)
                f.write('\n')
   ...
   #calculate expired time and write to error log
    ...
    try:
        ...
        detection_scores, map_locations = process_images(list_absolute_image_paths, 'MM-fusion_results/Coverage/')
        ...
        for image in original_images:
            ...
            with open('results_MM-fusion_Coverage.json', 'a') as f:
                json.dump(result_data, f)
                f.write('\n')
            mm_counter += 1
    ...
    try:
        ...
        for image in original_images:
            ...
            with open('results_DFAD2023_Coverage.json', 'a') as f:
                json.dump(result_data, f)
                f.write('\n')
            mm_counter += 1
