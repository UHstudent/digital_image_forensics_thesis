...
def save_ghostmaps(original, outputmaps):
    ...
    ghostmap_filepath = os.path.join('ghostmaps_results/CocoGlide', os.path.basename(original) + '_ghostmaps.png')
...
if __name__ == '__main__':
    ...
    with open('results_ghostmaps_CocoGlide.json', 'w') as f:
        json.dump({}, f)
    #create ghostmap output map
    os.makedirs('ghostmaps_results/CocoGlide', exist_ok=True)
    ...
    with open('results_probability_3x3_CocoGlide.json', 'w') as f:
        json.dump({}, f)
    #create probmap segmented 3x3 output map
    os.makedirs('probability_map_3x3_results/CocoGlide', exist_ok=True)
    ...
    with open('results_noise_CocoGlide.json', 'w') as f:
        json.dump({}, f)
    #create noise output map
    os.makedirs('noisemap_results/CocoGlide', exist_ok=True)
    ...
    with open('results_MM-fusion_CocoGlide.json', 'w') as f:
        json.dump({}, f)
    #create noise output map
    os.makedirs('MM-fusion_results/CocoGlide', exist_ok=True)
    ...
    with open('results_DFAD2023_CocoGlide.json', 'w') as f:
        json.dump({}, f)
    #create noise output map

    #create error log json file, so we can store results in file each time, preserving results in case of unexpected interupt
    with open('error_log_CocoGlide.json', 'w') as f:
        json.dump('hopefully this remains empty', f)
        f.write('\n')


    #cocoglide dataset contains pristine and manipulated images
    cocoglide_csv_path = "CocoGlide/table.csv"
    original_images = []
    manipulated_images = []
    masks = []

    with open(cocoglide_csv_path, newline='') as csvfile:
        csvreader = csv.reader(csvfile)
        next(csvreader)  #skip the first line
        for row in csvreader:
            original_images.append(os.path.join("CocoGlide",row[0]))
            manipulated_images.append(os.path.join("CocoGlide",row[1]))
            masks.append(os.path.join("CocoGlide",row[2]))
    ...
    try:
        ...
        for image_path in original_images:
            ...
            result_data = {
                ...
                "image outputmaps name": os.path.join('ghostmaps_results/CocoGlide', os.path.basename(image_path) + '_ghostmaps.png'),
            ...
            with open('results_ghostmaps_CocoGlide.json', 'a') as f:
        ...
        for image_path in manipulated_images:
            ...
            for i in range(n_ghostmaps-2*edge):
                ...
                if  len(values_inside_mask) == 0: #it can occur that the mask resizing deletes the mask area, in that= the entire mask becomes black or white; in such cases make top and bottom row different and log to error
                    ...
                    with open('error_log_CocoGlide.json', 'a') as f:
    ...
    for image_path in original_images:
        ...
        probmap_filepath = os.path.join('probability_map_3x3_results/CocoGlide', os.path.basename(image_path) + '_probmap.png')
        ...
        with open('results_probability_3x3_CocoGlide.json', 'a') as f:
            json.dump(result_data, f)
            f.write('\n')
    ...
    try:
        ...
        for image_path in original_images:
            ...
            noisemap_filepath = os.path.join('noisemap_results/CocoGlide', os.path.basename(image_path) + 'noisemap.png')
            ...
            noisemap_2_filepath = os.path.join('noisemap_results/CocoGlide', os.path.basename(image_path) + '_noisemap_2.png')
            ...
            with open('results_noise_CocoGlide.json', 'a') as f:
                json.dump(result_data, f)
                f.write('\n')
   ...
   #calculate expired time and write to error log
    ...
    try:
        ...
        detection_scores, map_locations = process_images(list_absolute_image_paths, 'MM-fusion_results/CocoGlide/')
        ...
        for image in original_images:
            ...
            with open('results_MM-fusion_CocoGlide.json', 'a') as f:
                json.dump(result_data, f)
                f.write('\n')
            mm_counter += 1
    ...
    try:
        ...
        for image in original_images:
            ...
            with open('results_DFAD2023_CocoGlide.json', 'a') as f:
                json.dump(result_data, f)
                f.write('\n')
            mm_counter += 1
